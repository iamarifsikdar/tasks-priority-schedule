  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 py-6 lg:py-10 max-w-4xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <EmptyState title="No activity yet" description="Send a test email or webhook to see logs here." />
      ) : (
          ...
          {logs.map((l) => (
            <div key={l.id} className="flex items-center gap-3 px-4 py-3 text-sm">
              <div className={cn(
                ...
                l.type === "email" ? "bg-priority-urgent-bg text-priority-urgent" : "bg-secondary",
              )}>
                {l.type === "email" ? <Mail className="h-4 w-4" /> : <Webhook className="h-4 w-4" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-medium capitalize">
                  {l.type} · {l.task_count} task{l.task_count === 1 ? "" : "s"}
                </div>
                <div className="text-xs text-muted-foreground">
                  {format(new Date(l.sent_at), "PPp")} · triggered by {l.triggered_by}
                </div>
                {l.error_message && (
                  <div className="text-xs text-destructive mt-0.5 truncate">{l.error_message}</div>
                )}
              </div>
              <span className={cn(
                "text-[11px] px-2 py-0.5 rounded-full font-semibold uppercase",
